//Contar todos los numeros desde a hasta b cuya suma de sus digitos sea un primo
#include <iostream>
#include<bits/stdc++.h>
...
int solveDP(int pos, int mayor, int digitos) {
    ...
    if(dp[pos][mayor][digitos]==-1) { //Preguntamos si es que no hemos calculado esta posicion antes
        ...
        int tope = 9;  //Si el mayor es false, significa que no existe numero con la posicion que tenemos que pueda sobrepasar a number, por lo que se queda en 9
        ...
        dp[pos][mayor][digitos] = 0; //Inicializamos todos los arreglos
